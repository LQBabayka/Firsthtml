...
console.log(Boolean(33));//true
    ...
    //Про переменные
//ссылки на массивы и объкеты:
let a = [1, 2, 3];//Объявим массив
let b = a;//в новый массив данне не скопировались, а создалась ссылка
console.log(a, b);//данные: 1, 2, 3
b.push(55); //b через ссылку обратилась к a, и уже в массиве a добавилась цифра 55
console.log(a, b);// одинаковые значения: 1,2,3,55
////
//Если проверяем значение переменной, объявленной через var до введеного значения, то будет возвращаться undefined.
//Если проверяем значение переменной, объявленной через let и const, то ничего не будет возвращаться, возникнет ошибка.
//Через let и const внутри блока можно обращаться и изменять объявленные ранее переменные, так же использовать нове, работающие только в этом блоке.
...
По соображениям безопасности JavaScript запрещает использование оператора ?? вместе с && и ||, если только приоритет явно не указан в круглых скобках.
            //Примеры всяких приколюх:
/*
//Типо калькулятор с суммой
let gh = prompt('введеные цифры будут суммироваться', 'введите число 1')
let gh1 = prompt('введеные цифры будут суммироваться', 'введите число 2')
gh = Number(gh);// приведение строки в число
gh1 = Number(gh1);
function sumpr(gh, gh1) {//объявление функции
  return gh + gh1;//то, что она будет делать
